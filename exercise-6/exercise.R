...
opp.teams <- husky.games.2015$opponent
...
max <- max(combined.yards)
most.yards <- husky.scores[max == combined.yards]
...
MostYardsScore <- function(games) {
  ...
  date.won <- date[combined.yards == max.yards]
  highest.score <- team.score[combined.yards == max.yards]
  return(paste("The best game of the Huskies was against", opponent.team, "on", date.won, 
...
highest.score <- MostYardsScore(husky.games.2016)
print(highest.score)